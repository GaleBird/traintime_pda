...
- `pubspec.yaml` 的 `version` 可保持 GXU 品牌语义版本（如 `1.0.0`），但 `+build` 必须单调递增（当前基线至少 `+50`），否则同包名安装会因 versionCode/build number 回退而覆盖失败。
...
- On this Windows machine, Android release builds can fail inside native-assets hook runner with `Could not find a command named "...\.dart_tool\hooks_runner\objective_c\<hash>\hook.dill"` when the generated `objective_c` hook cache is stale/half-generated; verify the path is under the repo, then remove `.dart_tool/hooks_runner/objective_c` and rebuild instead of changing source or dependencies.
...
- PigHub 上游当前图片列表接口是 `https://www.pighub.top/api/images?sort=2`，记录字段为 `image_url / filename / title / id`；不要再接回已返回 404 的 `/api/all-images`，字段缺失时应显式抛格式错误而不是生成 `"null"` 图片地址。
...
- GXU 成绩页的课程评价入口必须常驻在正常成绩页档案卡下方、搜索框上方，同时在 `checkdDycjd` 未放行的错误态保留同样的“官网评教 / 自动评教”入口；不要再只依赖成绩被拦截后才显示评教功能。入口卡片保持紧凑，只展示图标、`课程评价` 标题和两枚按钮，不再放“如果还有未完成评教...”这类说明段落；卡片背景应复用成绩条目卡片的轻染色口径，使用 `colorScheme.secondary.withValues(alpha: 0.08)` 的浅蓝/次强调底色和同色浅边框，不要退回灰色 `surfaceContainerHighest` 背景。官网评教必须打开研究生系统门户壳 `/view?m=up#act=yjs/py/kcpj/kcpjIndex?item_id=up_016_015`，不要直接打开模块内页 `/yjs/py/kcpj/kcpjIndex?...`，否则移动浏览器会显示缺少系统样式的原始页面；自动评教必须先弹出简短免责声明并要求用户勾选确认后，才允许通过 `GxuCourseEvaluationSession` 提交当前未完成的课程评价（`findKcpjPage` -> `findKcpjByJxjhxxid` -> 客观题最高分、主观题从内置正向评语池随机 -> `saveKdpj`）。免责声明只保留“客观题默认选择最高分、主观题随机正向评语”和“提交后通常不可撤回”两类关键信息，其中不可撤回提示需要用强调色/加粗突出；不要再把“会先检查课程评价状态...”放回免责声明。用户显式点击自动评教时，即使成绩当前已可查询，也必须实际执行评教流程，然后重新校验 `checkdDycjd == 1` 并刷新成绩；取消、未勾选、提交失败或校验仍未放行都必须显式停止/报错，不能伪造成绩可查。评教客观题分数 `fs` 必须可解析为数字才能参与最高分排序，不能把异常/缺失分数静默当作 0；`ypjsList` 既可能是教师/课程 id 字符串，也可能是含 `jszgh/kch/id/pjdxid` 的 map，必须正确识别已评对象以避免重复提交。
- 自动评教提交后若 `checkdDycjd` 仍未放行，必须继续抛 `GxuScoreEvaluationRequiredException`（定义在 `lib/repository/gxu_ids/gxu_score_exceptions.dart`），让成绩页保留“官网评教 / 自动评教”恢复入口；不要改回普通 `LoginFailedException` 或其它会进入通用错误页的异常类型。
- GXU 成绩页显式点击“自动评教”时，执行顺序必须先跑 `GxuCourseEvaluationSession.autoEvaluateRequiredCourses()`，再校验 `checkdDycjd`，之后再尝试成绩模块预热并刷新成绩，避免 `loadJxzlpj` 的初始化失败提前阻断评教；若评教后 `checkdDycjd == 1` 但预热仍返回 `0` 等异常，应带 warning 日志继续直接读取成绩单预览，并在成功 toast 中追加说明，不要再把用户送到“评教已完成但成绩初始化失败 0”的错误页。加载态需展示“正在检查课程评价并刷新成绩”，成功后根据 `GxuCourseEvaluationReport` toast 提示已提交/无待评/无需重复提交，若评教流程完成但成绩单预览仍失败，应抛 `GxuScoreAutoEvaluationRefreshException` 并保留评教报告信息用于明确提示。
- GXU 成绩页自动评教反馈不能只依赖短 toast：`GxuScoreSession.autoEvaluateAndFetchScoreSheet()` 的 `onEvaluationReport` 要在成绩刷新前回传评教结果，用于把加载态从“提交中”更新为“已全部完成/已提交若干门，正在刷新成绩”；显式点击自动评教时要先捕获稳定的根 `Navigator` 再切换 fetching 页面，否则按钮所在页面卸载后会丢失完成提示；成功后必须用结果弹窗明确提示，并在成绩页保留 `GxuAutoEvaluationFeedbackBanner` 常驻显示本次结果，尤其是“已确认课程评价全部完成，本次没有提交新评价，成绩已刷新。”这类再次点击自动评教的场景。
- GXU 成绩/选课错误态遇到研究生系统 `502 Bad Gateway` 时，必须通过 `describeGxuServiceError()` 显示“学校官网暂时无法访问”这类简洁用户提示，继续保留日志里的原始异常；不要再把 `DioException.toString()` / `Description:` 技术长文直接展示给用户。
- gxu.app 下载官网已迁移至 VPS `154.58.233.155`（2026-09-01 完成）：静态文件在宿主机 `/var/www/gxu.app/public`（由 `new-api-caddy` 容器以 `/var/www:/srv:ro` 只读挂载），Node 下载计数服务跑在 `gxu-app` 容器（`node:24-alpine`，监听 9080），数据落盘在宿主机 `/opt/gxu-app/data/download-counts.json`；两个容器都在 `new-api_new-api-network` 网络，Caddy 通过容器别名 `gxu-app:9080` 反代。
- `website/service/server.js` 默认监听 `127.0.0.1`，在容器里必须以 `HOST=0.0.0.0` 环境变量启动（上游代码未支持 HOST，部署时在 VPS 上打过补丁；上游若更新需带上该改动或上游化）。
- 反代配置在 VPS `/root/new-api/Caddyfile` 的 `gxu.app, origin.gxu.app { ... }` 站点块（line 93 附近）；Caddyfile 是只读 bind mount，改配置后必须删除并重建 `new-api-caddy` 容器（`docker cp` 到 bind mount 会报 device busy）。
- Cloudflare 侧有一个 Worker 拦截 `gxu.app/*` 并固定通过 `origin.gxu.app` 主机名回源（本机任何 CF API token 都无 Workers 权限，无法查看/修改该 Worker）。因此 `origin.gxu.app` 的 A 记录必须继续指向 `154.58.233.155` 且 Caddy 必须同时服务该主机名（证书已签发），否则 gxu.app 会出现 503/525。
- 更新清单仍由 `https://myapk.sgp1.cdn.digitaloceanspaces.com/manifests/update.json` 提供（`UPDATE_MANIFEST_URL` 环境变量），APK 直链在同一 Spaces；发布新版本时更新该 manifest 即可，无需改 VPS。
- gxu.app 官网的部署**唯一权威来源是 GitHub `GaleBird/gxu-app` 仓库**（`public/` + `service/`），不是本仓库的 `website/` 目录——后者已落后多个提交（缺 2026-06 之前的深色几何风前端重设计与新截图）。2026-09-02 已用 GitHub 最新版（c7b46c7）重新部署到 VPS 并全量验证。
- **上游陷阱：`GaleBird/gxu-app` 仓库 `deploy/gxu-app.env` 里的公钥是损坏的**（base64 某行 65 字符，Node/OpenSSL 报 `DECODER routines::unsupported`）。VPS 上 `/opt/gxu-app/gxu-app.env` 的公钥已改用本仓库 `lib/repository/fork_info.dart` 中验证过的正确公钥，`REQUIRE_MANIFEST_SIGNATURE=true` 已生效。如上游修复前重新生成 env，勿再从该文件复制公钥。
- VPS 上的 env 是 `--env-file` 传入的：改 `/opt/gxu-app/gxu-app.env` 后必须 `docker rm -f` + `docker run` 重建 gxu-app 容器，`docker restart` 不会重读 env 文件；docker env-file 不支持多行值，PEM 必须压成 `\n` 转义的单行。
- 2026-09-02 官网加载提速：VPS 上 `icon.png` 已从 1.4MB 压到 141KB（512px + pngquant，原图备份 `/root/icon.png.bak`），9 个 HTML 的引用版本串统一为 `?v=20260902-1`；Caddy 给静态扩展（css/js/png/jpg/svg 等）加了 `Cache-Control: public, max-age=604800`（Caddyfile 备份 `/root/Caddyfile.bak-20260902`）。**注意上游 gxu-app 仓库里仍是 1.4MB 原图，将来从仓库重新部署会退化，需先把压缩版提交回上游或部署后重打补丁。**
- 官网剩余瓶颈是 HTML 不被 CF 缓存（`CF-Cache-Status: DYNAMIC`，每次请求 Worker 都回源东京，TTFB ~1s）：需要用户在 Cloudflare 仪表盘手动加 Cache Rule（本机 token 无权限）——匹配 gxu.app 且路径不含 /api/、/download/，Edge TTL 设几分钟。源站本身 TTFB 仅 17ms，不是瓶颈。
- 2026-09-02 官网边缘缓存：CF Worker `gxu-maintenance`（路由 `gxu.app/*`）已升级为带 Cache API 的版本——GET 且路径以 `/` 或 `.html` 结尾的页面在边缘缓存 300s（命中时返回 `X-Edge-Cache: HIT` 和 `Cache-Control: public, max-age=300`），`/api/*`、`/download/*` 维持 `no-store` 直通，4s 超时+维护页逻辑原样保留。改动原因：Worker 接管的请求会绕过 Cache Rules，此前创建的 zone 级 Cache Rule（HTML 缓存 5 分钟）对该路由不生效（保留着，将来若删掉 Worker 路由它会自动接管）。Worker 原版源码备份在 VPS `/root/gxu-maintenance-original-backup.js`，新版在 `/root/gxu-maintenance-v2-caching.js`；上传 Worker 新版本时必须在 metadata 里带上 `bindings: [{ORIGIN_HOST: origin.gxu.app}]` 和 `compatibility_date: 2026-03-25`，否则绑定丢失站点直接 500。
- CF 令牌管理：用户持有可创建任意权限令牌的主令牌（token-creation token）；本次为操作创建过 Workers Scripts Read/Write 令牌（保留），cache-rules 令牌用完已删。令牌值不写入本文件，需要时用主令牌重新铸造。
- 2026-09-05 下载源迁移：DigitalOcean Spaces 已失效，APK 下载与更新清单已整体迁到 Cloudflare R2。R2 桶 `gxu-apk`（APAC，account `e88618e67ca2d1284aa29f8a499c9da2`），绑定自定义域 `dl.gxu.app`（gxu.app zone 内，CF 自动 DNS）。公开读走 `https://dl.gxu.app/releases/<tag>/app-<abi>-release.apk` 与 `https://dl.gxu.app/manifests/update.json`。
- R2 上传鉴权方式（关键、非标准）：不用 Dashboard 的 Access Key，而是用铸造令牌创建的 bucket-scoped user token——**Access Key ID = token 的 UUID（32 hex），Secret Access Key = sha256(token value)**。上传脚本 `tool/generators/r2_upload.py`（SigV4 手写实现 + >20MB 自动 multipart，每块 20MB）；S3 canonical URI 里 tag 名的 `+` 必须 urlencode 成 `%2B`，否则 SignatureDoesNotMatch。
- CI workflow `release_for_android.yaml` 的 Spaces 上传步骤已替换为 `r2_upload.py` 调用，需要 4 个新 GitHub Secrets：`R2_ACCOUNT_ID`、`R2_TOKEN_ID`（token UUID）、`R2_TOKEN_VALUE`（cfut_ 值）、`R2_CDN_BASE_URL`（`https://dl.gxu.app`）。旧的 DO_SPACES_* Secrets 可删。
- manifest 签名私钥只在 GitHub Secrets（本地无副本），所以 R2 上首版 manifest 必须由 CI 发版时生成上传；v1.0.6+50 的 3 个 APK 已手动从 GitHub Release 下载并上传到 R2 作为过渡下载源（`releases/v1.0.6+50/`）。
- `lib/repository/fork_info.dart` 的 manifest 地址与受信 host 已切到 `dl.gxu.app`（`updateManifestDownloadHost`，原 `updateManifestSpacesHost` 改名；`trustedSpacesReleasePathPrefix` → `trustedDownloadReleasePathPrefix`）。存量已装用户在装到指向 R2 的新版之前，检查更新会失败（Spaces 403），需引导官网手动下载。
- VPS `/opt/gxu-app/gxu-app.env` 已加 `UPDATE_MANIFEST_URL=https://dl.gxu.app/manifests/update.json` 与 `ALLOWED_DOWNLOAD_HOSTS=dl.gxu.app,github.com`，gxu-app 容器已按约定 `docker rm -f` + `docker run` 重建生效。注意 `GaleBird/gxu-app` 上游仓库的 config.js 仍指 Spaces，需同步提交更新。
- R2 对象的 Content-Type/Cache-Control 必须在 multipart init 或单次 PUT 时带；漏了可用 `PUT + x-amz-copy-source 自身 + x-amz-metadata-directive: REPLACE` 服务器端自拷贝补齐。CF 边缘会缓存无头旧响应几分钟，验证时加 `?nocache=` 参数绕过。
- 2026-09-05 v1.0.7+51 已发布（R2 迁移后首个版本）：CI 构建成功，manifest（RSA 签名）已上传 `dl.gxu.app/manifests/update.json`，官网 `/api/update` 与 `/download/*`（302 → R2）已全部恢复。存量 v1.0.6 及以下用户需从官网手动下载一次本版，之后应用内检查更新恢复。
- CI Kotlin 构建坑（2026-09-05 首次发版失败原因）：`home_widget 0.8.1` 报 "Cannot inline bytecode built with JVM target 11 into JVM target 1.8"。修复在 `android/build.gradle` 的 `subprojects.afterEvaluate`：对 Kotlin 插件子项目统一 `kotlinOptions.jvmTarget = "17"`，并在 `project.android.compileOptions` 层同步设 Java 17（只在 JavaCompile task 上设 `sourceCompatibility` 不生效，仍会报 Java 1.8 / Kotlin 17 不一致）。本地验证 `flutter build apk --debug` 通过后再推 tag。
- 本机 `D:\Android\.gradle\wrapper\dists\gradle-8.11.1-bin` 曾损坏（缺 kotlin-compiler-embeddable jar，报 IllegalArgumentException），删掉整个 dist 目录让 wrapper 重下即恢复。
- 2026-09-05 manifest 多源回退（根治"存储再挂更新就坏"）：app 端 `pda_service_session.dart` 检查更新改为依次尝试主源 `dl.gxu.app/manifests/update.json` → 回退源 `gxu.app/api/manifest`（列表在 `fork_info.dart` 的 `updateManifestFallbackUrls`）。任一源 404 视为 noRelease；网络错误换下一源。回退源是官网 VPS 透传路由（上游 `GaleBird/gxu-app` 仓库 `service/server.js` 的 `/api/manifest`，commit a4c9dd6），从 `UPDATE_MANIFEST_URL` env 拉取并原样返回——**将来存储再迁移只需在 VPS 改这个 env + `docker rm -f` 重建容器，存量用户无需发版**。注意 `_fetchManifestWithFallback` 返回类型必须写 `Future<dynamic>`（dio 和 get 都导出 `Response`，写 `Response<dynamic>` 会 ambiguous_import）。
- 上游 gxu-app 的 `service/config.js` 已上游化 `host: process.env.HOST || "127.0.0.1"`（commit 37b9201），替代 VPS 上手打的 HOST=0.0.0.0 补丁；从仓库重新部署不会再漏 HOST 导致官网 503。但 config.js 的 manifest 默认值仍指已失效的 Spaces，新环境部署必须带 `UPDATE_MANIFEST_URL=https://dl.gxu.app/manifests/update.json` env（VPS 靠 env 覆盖所以线上正常）。
- 2026-09-05 v1.0.7+51 删发重发：因首版未含 manifest 回退逻辑，已删除 GitHub Release（API `DELETE /repos/GaleBird/traintime_pda/releases/{id}`，release id 382917473）+ 远端/本地 tag，带多源回退改动后重新打 tag 触发 CI 重发（R2 对象同 key 直接覆盖，无需清理）。删除 release 用 `git credential fill` 拿 GitHub token 走 API。
- VPS manifest 透传路由加了 60s 内存缓存 + 失败回退旧缓存（上游 gxu-app `service/manifest-client.js`，commit 5272d44，测试 7731af2，`MANIFEST_CACHE_TTL_MS` env 可调）：此前每次请求都实时回源 dl.gxu.app，CF 边缘缓存 miss 时可能超过 Worker 4s 超时触发维护页 503；现在慢/挂时直接吐上一份好 manifest，回退源真正抗抖动。部署方式：scp `config.js` + `manifest-client.js` 到 `/opt/gxu-app/service/`（容器只读 bind mount，代码改动 `docker restart` 即可，env 改动才需要 rm -f 重建）。
- VPS gxu-app 容器 9080 端口**没有发布到宿主机**（Caddy 走 `new-api_new-api-network` 容器网络直连 `gxu-app:9080`），宿主机 `curl 127.0.0.1:9080` 会连接拒绝——容器内验证用 `docker exec gxu-app sh -c 'wget -qO- http://127.0.0.1:9080/api/manifest'`。
- 本机 `D:\Android\.gradle\wrapper\dists\gradle-8.11.1-bin` 曾损坏（缺 kotlin-compiler-embeddable jar，报 IllegalArgumentException），删掉整个 dist 目录让 wrapper 重下即恢复。
