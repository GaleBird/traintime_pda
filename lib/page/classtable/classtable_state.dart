...
class ClassTableWidgetState with ChangeNotifier {
  ...
  final ExamController? examController = Get.isRegistered<ExamController>()
      ? Get.find<ExamController>()
      : null;
  final ExperimentController? experimentController =
      Get.isRegistered<ExperimentController>()
  ...
  set chosenWeek(int chosenWeek) {
    if (chosenWeek == _chosenWeek) return;
  ...
  /// The exam list.
  List<Subject> get subjects => examController?.data.subject ?? const [];

  /// The experiment list.
  List<ExperimentData> get experiments =>
      experimentController?.data ?? const [];
...
''';
  ...
  Future<void> updateClasstable(BuildContext context) async {
    ...
    if (examController != null) {
      futures.add(examController!.get());
    }
    if (experimentController != null) {
      futures.add(experimentController!.get());
    }
